...
class TypeclassSolverSpec {
    ...
    @Test fun `seedless transitive class fails fast`() {
        ...
        val e = assertFailsWith<InstanceNotFoundException> {
            st.resolve(listOf(goal))
        }
    ...
    @Test fun `variant cycles pruned but facts still reachable`() {
    ...
    class InstanceTestModule {
        ...
        @Mu.Export fun <A> list(@Mu.ZeroOrMore @Mu.Name("elements") elements: List<A>): List<A> = elements

        @Mu.Instance
        fun <A, B> showViaUpcast(
            @Mu.Instance up: Upcast<A, B>,
            @Mu.Instance sb: Show<B>
        ): Show<A> =
            object : Show<A> {
                override fun show(a: A): String = sb.show(up(a)) // use the Upcast to convert, then show
            }
    }
