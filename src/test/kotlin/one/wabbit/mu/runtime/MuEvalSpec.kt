...
class BuiltinsModule {
    ...
    @Mu.Export
    fun incrementCounter(): BigInteger {
    ...
    @Mu.Export(":")
    fun <A, B> makePair(@Mu.Name("key") a: A, @Mu.Name("value") b: B): Pair<A, B> = Pair(a, b)

    @Mu.Export("set-of")
    fun <A> makeSet(@Mu.Name("key") @Mu.ZeroOrMore values: List<A>): Set<A> = values.toSet()

    @Mu.Export("list-of")
    fun <A> makeList(@Mu.Name("key") @Mu.ZeroOrMore values: List<A>): List<A> = values
    ...
    @Mu.Instance
    fun <A> numericCastIdentity(): NumericCast<A, A> =
        object : NumericCast<A, A> {
            override fun upcast(a: A): A = a
        }

    @Mu.Instance
    fun <A, B, C> composeNumericCast(
        a2b: NumericCast<A, B>,
        b2c: NumericCast<B, C>,
    ): NumericCast<A, C> =
        object : NumericCast<A, C> {
    ...
    @Mu.Instance
    val castRationalToDouble =
        object : NumericCast<Rational, Double> {
            override fun upcast(a: Rational): Double = a.toDouble()
        }

    @Mu.Instance
    val bigIntegerGroup =
        object : Group<BigInteger> {
    ...
    @Mu.Instance
    fun <A> monoidFromGroup(m: Group<A>): Monoid<A> =
        object : Monoid<A> {
    ...
    @Mu.Instance
    fun <A, B, C> upcastCompose(a2b: Upcast<A, B>, b2c: Upcast<B, C>): Upcast<A, C> =
        Upcast.of { b2c(a2b(it)) }

    @Mu.Instance
    fun <A1, A2, B> upcastUnion2_1(a: Upcast<A1, A2>): Upcast<Union2<A1, B>, Union2<A2, B>> =
        Upcast.of { it.map1(a::invoke) }

    @Mu.Instance
    fun <A1, A2, B> upcastUnion2_2(a: Upcast<A1, A2>): Upcast<Union2<B, A1>, Union2<B, A2>> =
        Upcast.of { it.map2(a::invoke) }

    @Mu.Instance fun <A, B> union2_U1(): Upcast<A, Union2<A, B>> = Upcast.of { Union2.U1(it) }

    @Mu.Instance fun <A, B> union2_U2(): Upcast<A, Union2<B, A>> = Upcast.of { Union2.U2(it) }

    @Mu.Instance
    fun <A, B> upcastList(a: Upcast<A, B>): Upcast<List<A>, List<B>> =
        Upcast.of { it.map(a::invoke) }

    // Upcasts for MuLiteralInt
    @Mu.Instance
    val upcastMuLiteralIntToBigInteger = Upcast.of<MuLiteralInt, BigInteger> { it.value }

    @Mu.Instance
    val upcastMuLiteralIntToInt =
        Upcast.of<MuLiteralInt, Int> {
            try {
                it.value.intValueExact()
            } catch (e: ArithmeticException) {
                throw MuException("Integer literal ${it.value} too large for Int", e)
            }
        }

    // Upcasts between standard numeric types (needed for '+')
    @Mu.Instance
    val upcastIntToBigInteger = Upcast.of<Int, BigInteger> { BigInteger.valueOf(it.toLong()) }
    ...
    @Mu.Instance
    val upcastRationalToDouble =
        Upcast.of<Rational, Double> { it.toDouble() } // Already exists via NumericCast
        ...
        when (params) {
            ...
            is MuExpr.Atom -> {
                ...
                val evalResult =
                    try {
                        ...
                        throw MuException(
                            "Error evaluating definition body for '$name': ${e.message}",
                            e,
                        )
                    }
            ...
            is MuExpr.Seq -> {
                ...
                if (funcNameExpr !is MuExpr.Atom) {
                    throw MuException(
                        "Invalid function definition: Function name must be an atom, got $funcNameExpr."
                    )
                }
                ...
                if (signatureParts.isEmpty()) {
                    throw MuException(
                        "Invalid function definition '$name': Missing parameter list and/or return type."
                    )
                }
                ...
                for (paramDef in paramDefsExpr) {
                    if (
                        paramDef !is MuExpr.Seq ||
                            paramDef.value.size != 2 ||
                            paramDef.value[0] !is MuExpr.Atom
                    ) {
                        throw MuException(
                            "Invalid parameter definition in function '$name': Expected (paramName Type), got $paramDef."
                        )
                    }
                    ...
                    val (_, paramTypeVal) =
                        try {
                            ...
                            throw MuException(
                                "Error evaluating parameter type '$paramTypeExpr' for '$paramName' in function '$name': ${e.message}",
                                e,
                            )
                        }
                    val paramType =
                        paramTypeVal.unsafeValue as? MuType
                            ?: throw MuException(
                                "Parameter type expression '$paramTypeExpr' for '$paramName' in function '$name' did not evaluate to a Type."
                            )
                ...
                val (_, returnTypeVal) =
                    try {
                        ...
                        throw MuException(
                            "Error evaluating return type '$returnTypeExpr' for function '$name': ${e.message}",
                            e,
                        )
                    }
                val returnType =
                    returnTypeVal.unsafeValue as? MuType
                        ?: throw MuException(
                            "Return type expression '$returnTypeExpr' for function '$name' did not evaluate to a Type."
                        )
                ...
                val value =
                    MuStdValue.func(
                        ...
                        parameters.map {
                            Arg(it.first, false, ArgArity.Required, it.second)
                        }, // Assume required/non-quoted for now
                        returnType,
                    ) { callContext, args ->
                        // Create a new scope for the function call with arguments bound
                        val functionScopeCtx =
                            definitionContext.withLocals(args) // Use definition context as base
                        ...
                        return@func try {
                            ...
                            throw MuException(
                                "Error evaluating body of function '$name': ${e.message}",
                                e,
                            )
                        }
                    }
            ...
            // Invalid definition format
                ...
                throw MuException(
                    "Invalid definition format: Expected atom or sequence for definition head, got $params."
                )
            }
        }
    }
...
class MuEvalSpec {
    ...
    fun test() {
        ...
        // FIXME: fails with Instance not found for goal:
        // Constructor(head=one.wabbit.mu.types.Upcast,
        // args=[Constructor(head=one.wabbit.math.Rational, args=[]),
        // Constructor(head=java.math.BigInteger, args=[])])
        //        wtf?
    ...
    private fun assertEval(
        ...
        val (newCtx, actualValue) =
            try {
                evaluateMu(initialCtx, expr, MuStdContext)
            } catch (e: Exception) {
                fail(
                    "Evaluation failed for '$expr': ${e::class.simpleName} - ${e.message}\n$message",
                    e,
                )
            }
    ...
    private inline fun <reified T : Throwable> assertEvalFails(
        ...
        val exception =
            assertFailsWith<T>("Expected ${T::class.simpleName} for '$expr'. $message") {
                evaluateMu(initialCtx, expr, MuStdContext)
            }
        if (messageContains != null) {
            assertTrue(
                exception.message?.contains(messageContains) ?: false,
    ...
    @Test
    fun `evaluate literals and basic types`() {
    ...
    fun `evaluate arithmetic operations`() {
        ...
        ctx =
            assertEval(
                ...
                MuType.BigInteger,
                BigInteger("1000000000000000001"),
                "BigInt + Int",
            )
        // Rational + Int -> Rational (via LitInt->Rational, Int->Rational upcasts + Rational
        // Monoid)
        // FIXME: fails
        // ctx = assertEval(ctx, "(+ 1/2 1)", MuType.Rational, Rational.from(3, 2), "Rational +
        // Int")
    ...
    @Test
    fun `evaluate stateful operations`() {
        var ctx = setupContext()
        // Counter
        ...
        // ctx = assertEval(ctx, "(counter 50)", MuType.BigInteger, BigInteger.ZERO, "Set counter,
        // returns old") -> This depends on mutable prop implementation detail
        // assertEquals(BigInteger("50"),
        // (ctx.modules["builtins"]!!.definitions["counter"]!!.unsafeValue as MuStdFunc). ??? ) //
        // Need way to check module state
        ...
        ctx =
            assertEval(
                ...
                MuType.BigInteger,
                BigInteger.TWO,
                "Counter after second increment",
            )
        ...
        ctx =
            assertEval(
                ...
                MuType.Int.nullable(),
                0,
                "Set nullableInt, returns old value",
            )
        // Check new value
        ctx =
            assertEval(ctx, "(nullableInt)", MuType.Int.nullable(), 10, "nullableInt after setting")
        // Set nullableInt to null (pass nothing to optional arg)
        // How to represent null explicitly in Mu syntax? Assume setting requires a value or use a
        // specific 'set-null' function if needed.
        // Let's assume setting to null isn't directly supported by `(nullableInt)` call without
        // args yet.
        ...
        ctx =
            assertEval(
                ...
                MuType.Int.nullable(),
                10,
                "Set nullableInt again, returns old value",
            )
        ctx =
            assertEval(
                ctx,
                ...
                MuType.Int.nullable(),
                5,
                "nullableInt after setting again",
            )
    }

    @Test
    fun `evaluate collection operations`() {
        var ctx = setupContext()
        // list
        ctx =
            assertEval(
                ctx,
                "[1 2 3]",
                MuType.List(MuType.lift<MuLiteralInt>()),
                listOf(
                    MuLiteralInt(BigInteger.ONE),
                    MuLiteralInt(BigInteger.TWO),
                    MuLiteralInt(BigInteger("3")),
                ),
                "List of LitInt",
            )
        // list-of (explicit)
        ...
        ctx =
            assertEval(
                ctx,
                "(set-of 1 2 1 3)",
                MuType.Set(MuType.lift<MuLiteralInt>()),
                setOf(
                    MuLiteralInt(BigInteger.ONE),
                    MuLiteralInt(BigInteger.TWO),
                    MuLiteralInt(BigInteger("3")),
        ...
        ctx =
            assertEval(
                ...
                mapOf(
                    MuLiteralInt(BigInteger.ONE) to MuLiteralString("a"),
                    MuLiteralInt(BigInteger.TWO) to MuLiteralString("b"),
                ),
                "Map LitInt -> LitString",
            )
        // FIXME: FAILS by producing UNBOUND TYPES????
        // ctx = assertEval(ctx, "(map-of)", MuType.Map(MuType.Nothing, MuType.Nothing),
        // emptyMap<Any?, Any?>(), "Empty map-of")
        ...
        // ctx = assertEval(ctx, "[1 2 3.0]", MuType.List(MuType.Double), listOf(1.0, 2.0, 3.0),
        // "List LitInt + Double -> List Double")
        // [1 1/2] -> liftList should find common type Rational
        // ctx = assertEval(ctx, "[1 1/2]", MuType.List(MuType.Rational), listOf(Rational.from(1),
        // Rational.from(1, 2)), "List LitInt + Rational -> List Rational")
        // [1 1/2 3.0] -> liftList should find common type Double
        // ctx = assertEval(ctx, "[1 1/2 3.0]", MuType.List(MuType.Double), listOf(1.0, 0.5, 3.0),
        // "List LitInt + Rational + Double -> List Double")
        // ["a" 1] -> liftList should find common type Any (or fail if no Upcast<String,Any>,
        // Upcast<LitInt,Any>)?
        // Current liftList might find Union or specific common supertype if instances exist. Let's
        // assume Any for now.
        // This requires Upcast<MuLiteralString, Any> and Upcast<MuLiteralInt, Any>
        // Add dummy Upcast<..., Any> for testing this:
        var ctxWithAny =
            ctx.withNativeModule(
                "anyUp",
                object {
                    @Mu.Instance fun <T> upcastToAny() = Upcast.of<T, Any> { it as Any }
                },
            )
        // FIXME: fails by choosing Union2<MuLiteralString, MuLiteralInt> instead of Any. Not
        // exactly a failure??
        // ctxWithAny = assertEval(ctxWithAny, "[\"a\" 1]", MuType.List(MuType.lift<Any>()),
        // listOf(MuLiteralString("a"), MuLiteralInt(BigInteger.ONE)), "List String + LitInt -> List
        // Any")
    }

    @Test
    fun `evaluate define operations`() {
        var ctx = setupContext()
        // Define variable
        ...
        ctx =
            assertEval(
                ctx,
                "x",
                MuType.lift<MuLiteralInt>(),
                MuLiteralInt(BigInteger.TEN),
                "Evaluate defined variable x",
            )
        ...
        ctx =
            assertEval(
                ...
                "Call defined function double",
            )
        // FIXME: FAILS
        // ctx = assertEval(ctx, "(double x)", MuType.BigInteger, BigInteger.valueOf(20), "Call
        // defined function with defined var") // x is 10 (Int)
        // Define function using other defined function
        ctx =
            assertEval(
                ctx,
                "(define (quad (n Int) Int) (double (double n)))",
                MuType.Unit,
                Unit,
        ...
        ctx =
            assertEvalFails<MuException>(
                ctx,
                "(define)",
                message = "Missing params/body",
                messageContains = "Missing required argument:",
            )
        ctx =
            assertEvalFails<MuException>(
                ctx,
        ...
        ctx =
            assertEvalFails<MuException>(
                ctx,
                "(define (f x Int) x)",
                message = "Bad param def",
                messageContains = "Invalid parameter definition",
            )
        ctx =
            assertEvalFails<MuException>(
                ctx,
                "(define (f (x BadType) Int) x)",
                message = "Bad param type",
                messageContains = "Error evaluating parameter type",
            )
        ctx =
            assertEvalFails<MuException>(
                ctx,
                "(define z (/ 1 0))",
                message = "Bad var body eval",
                messageContains = "Error evaluating definition body",
            ) // Error during value evaluation
    }
}
