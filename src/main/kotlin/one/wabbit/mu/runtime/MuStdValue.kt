...
data class MuStdValue private constructor(val unsafeValue: Any?, val type: MuType) {
    override fun toString(): String = "$unsafeValue : ${TypeFormatter.default.format(type)}"
    ...
    companion object {
         ...
         *
         * This provides a simpler way to expose Kotlin functions compared to full module reflection
         * with `withNativeModule`.
         *
         * Limitations:
         * - Does NOT support generic functions (KFunction with type parameters).
         * - Does NOT support receiver parameters (instance or extension). Function must be
         *   top-level or bound.
         ...
         * @throws IllegalArgumentException if the function is generic, requires context/instance
         *   params, or parameters lack names.
         */
        fun liftKFunction(
            kfun: KFunction<*>,
            ...
            if (kfun.typeParameters.isNotEmpty()) {
                throw IllegalArgumentException(
                    "liftKFunction does not support generic functions: ${kfun.name}"
                )
            }

            val muParams = mutableListOf<Arg<MuStdValue>>()
            val kotlinParamsData =
                mutableListOf<Pair<KParameter, MuType>>() // Store KParam and MuType

            for (p in kfun.parameters) {
                // Limitation: Skip receiver parameters
                if (
                    p.kind == KParameter.Kind.INSTANCE ||
                        p.kind == KParameter.Kind.EXTENSION_RECEIVER
                ) {
                    if (p.kind == KParameter.Kind.INSTANCE && receiver == null) {
                        throw IllegalArgumentException(
                            "Receiver parameter found but no receiver instance provided for function ${kfun.name}"
                        )
                    }
                    //                    if (p.kind == KParameter.Kind.INSTANCE && receiver != null
                    // && !p.type.classifier) {
                    //                        throw IllegalArgumentException("Provided receiver type
                    // mismatch for function ${kfun.name}")
                    //                    }
                    continue
                }
                ...
                if (
                    p.type.classifier == MuStdContext::class ||
                        p.annotations.any { it is Mu.Instance }
                ) {
                    throw IllegalArgumentException(
                        "liftKFunction cannot handle @Context or @Instance parameters: ${p.name} in ${kfun.name}. Use withNativeModule."
                    )
                }
                ...
                if (
                    ...
                    println(
                        "Warning: liftKFunction ignores @Optional, @ZeroOrMore, @OneOrMore, @Quoted, @Name annotations on parameter ${p.name} in ${kfun.name}."
                    )
                }
                ...
                val pName =
                    p.name
                        ?: throw IllegalArgumentException(
                            "KFunction parameters must have names for lifting (function: ${kfun.name})"
                        )
                val pType =
                    MuType.fromKType(p.type) // No bound vars needed as we disallowed generics
            ...
            val runLambda:
                (MuStdContext, Map<String, MuStdValue>) -> Pair<MuStdContext, MuStdValue> =
                { ctx, argMap ->
                    ...
                    kotlinParamsData.forEach { (kParam, muType) ->
                        val muArg =
                            argMap[kParam.name!!]
                                ?: throw MuException(
                                    "Missing argument '${kParam.name!!}' for lifted function '$name'"
                                )
                        ...
                        //    throw MuException("Type mismatch for argument '${kParam.name!!}'.
                        // Expected compatible with $muType, got ${muArg.type}")
                        // }
                    ...
                    try {
                        // Use callBy for KFunctions >= 1.3.70 for potential parameter reordering
                        // resilience,
                        // but requires mapping KParameter -> Value. Sticking to positional call()
                        // for simplicity.
                        ...
                        throw MuException(
                            "Error calling lifted Kotlin function '$name': ${e.message}",
                            e,
                        )
                    }
                }
