...
open class UnificationException(val left: MuType, val right: MuType, message: String) :
...
class TypeMismatchException(val expected: MuType, val actual: MuType) :
    TypeInferenceException(
        "type mismatch: expected ${F.format(expected)}, found ${F.format(actual)}"
    )

/** Occurs check (infinite type). */
class OccursCheckException(val variable: TypeVariable, val inType: MuType) :
    TypeInferenceException("occurs check failed: $variable occurs in ${F.format(inType)}")

/** Base exception for errors during type class instance resolution. */
open class InstanceResolutionException(message: String, cause: Throwable? = null) :
    MuException(message, cause)
...
class InstanceNotFoundException(
    ...
    InstanceResolutionException(
        "Instance not found for goal after $iterations/$maxIterations: $goal${context?.let { " (context: $it)" } ?: ""}"
    )
...
class AmbiguousInstanceException(
    ...
    InstanceResolutionException(
        "Ambiguous instance resolution for goal: $goal. Potential instances: $instances"
    )
...
class CyclicInstanceDependencyException(
    ...
    InstanceResolutionException(
        "Cyclic instance dependency detected for goal: $goal. Path: ${path.joinToString(" -> ")}"
    )
