...
import one.wabbit.mu.types.MuType.Constructor
import one.wabbit.mu.types.MuType.Exists
import one.wabbit.mu.types.MuType.Forall
import one.wabbit.mu.types.MuType.Func
import one.wabbit.mu.types.MuType.Implicit
import one.wabbit.mu.types.MuType.Use

/**
 * Formats MuType objects into human-readable strings. Allows customization of how type constructor
 * names are displayed.
 */
class TypeFormatter(private val rewrites: Map<String, RewrittenName>) {
    ...
    private fun formatConstructor(type: Constructor): String {
        ...
        if (type.head == "?") {
            ...
            require(type.args.size == 1) {
                "Nullable constructor '?' must have exactly one argument."
            }
            ...
            val requiresParens =
                firstArg is Func ||
                    firstArg is Forall ||
                    (firstArg is Constructor &&
                        firstArg.head == "?") // Avoid double '??' without parens
        ...
        val rewrite =
            rewrites[type.head]
                ?: RewrittenName.Name(type.head) // Default to Name if no rewrite found
        ...
        return when (rewrite) {
            is RewrittenName.Name -> {
                if (type.args.isEmpty()) {
                    ...
                    "${rewrite.name}[${type.args.joinToString(", ") { format(it) }}]"
                }
            }
            is RewrittenName.Infix -> {
                check(type.args.size == 2) {
                    "Infix operator '${rewrite.symbol}' requires 2 arguments, got ${type.args.size} for head '${type.head}'"
                }
                val firstArg = type.args[0]
            ...
            is RewrittenName.Postfix -> {
                check(type.args.size == 1) {
                    "Postfix operator '${rewrite.symbol}' requires 1 argument, got ${type.args.size} for head '${type.head}'"
                }
                val firstArg = type.args[0]
            ...
            is RewrittenName.Prefix -> {
                check(type.args.size == 1) {
                    "Prefix operator '${rewrite.symbol}' requires 1 argument, got ${type.args.size} for head '${type.head}'"
                }
                val firstArg = type.args[0]
                // Add parentheses for precedence
                val arg = formatParenthesized(firstArg, rewrite)
    ...
    private fun formatImplicit(type: Implicit): String =
        "forall ${type.typeParameters.joinToString(", ")}. ${type.parameters.joinToString(", ")} => ${format(type.returnType)}"

    private fun formatForall(type: Forall): String =
        "forall ${type.vars.joinToString(", ")}. ${format(type.tpe)}"

    private fun formatExists(type: Exists): String =
        "exists ${type.vars.joinToString(", ")}. ${format(type.tpe)}"
    ...
    companion object {
        ...
        val DEFAULT_REWRITES: Map<String, RewrittenName> =
            mapOf(
                ...
                MuLiteralString::class.qualifiedName!! to
                    RewrittenName.Name("LitString"), // Added LitString
                MuType::class.qualifiedName!! to RewrittenName.Name("Type"), // Added Type itself
                MuStdContext::class.qualifiedName!! to
                    RewrittenName.Name("Context"), // Added Context
                ...
                // "?" to RewrittenName.Postfix("?"), // This rewrite isn't directly used due to
                // special handling
            )
